...
Проект представляет из себя фреймворк UI-тестирования платформы, имитирующей работу интернет-магазины одежды [Swag Labs](https://www.saucedemo.com/). 
...
Объектом тестирования является работа сервиса преимущественно со стороны взаимодействия пользователя через UI, желающего заказать онлайн брендовый товар магазина Swag Labs 

Затронуты следующие модули сайта:

`Авторизация пользователя` - начальная страница при переходе на сайт

`Основной каталог товаров` - страница для просмотра ассортимента товаров и добавления их в корзину

`Корзина` - страница корзины, позволяет сверить список покупок перед началом оформления заказа

`Оформление заказа` - разделен на 2 этапа: ввод данных заказчика (ФИО и почтоный индекс) и подтверждение заказа

Тестовые сценарии находятся в файле [Swag Scenarios](./Swag Scenarios.txt)

Тестирование не затрагивает:
- страницу лендинга всей платформы Souce
- saadsa
- asdasd
- adada
